...
class Str {
  ...
  getPublicKey(path, boolValidate, boolDisplay) {
    ...
    let apdus = [];
    let response;
    let pathElts = (0, _utils.splitPath)(path);
    ...
    let keepAlive = false;
    return (0, _utils.foreach)(apdus, data => this.transport.send(CLA, keepAlive ? INS_KEEP_ALIVE : INS_GET_PK, boolValidate ? 0x01 : 0x00, boolDisplay ? 0x01 : 0x00, data, [SW_OK, SW_KEEP_ALIVE]).then(apduResponse => {
      let status = Buffer.from(apduResponse.slice(apduResponse.length - 2)).readUInt16BE(0);
      ...
      offset += 32;
      let publicKey = (0, _utils.encodeEd25519PublicKey)(rawPublicKey);
  ...
  signTransaction(path, transaction) {
    ...
    let keepAlive = false;
    return (0, _utils.foreach)(apdus, (data, i) => this.transport.send(CLA, keepAlive ? INS_KEEP_ALIVE : INS_SIGN_TX, i === 0 ? P1_FIRST_APDU : P1_MORE_APDU, i === apdus.length - 1 ? P2_LAST_APDU : P2_MORE_APDU, data, [SW_OK, SW_CANCEL, SW_UNKNOWN_OP, SW_MULTI_OP, SW_KEEP_ALIVE]).then(apduResponse => {
      let status = Buffer.from(apduResponse.slice(apduResponse.length - 2)).readUInt16BE(0);
      ...
      if (status === SW_OK) {
        ...
        return this.signHash_private(path, (0, _utils.hash)(transaction));
  ...
  signHash_private(path, hash) {
    ...
    let keepAlive = false;
    return (0, _utils.foreach)(apdus, data => this.transport.send(CLA, keepAlive ? INS_KEEP_ALIVE : INS_SIGN_TX_HASH, 0x00, 0x00, data, [SW_OK, SW_CANCEL, SW_NOT_ALLOWED, SW_UNSUPPORTED, SW_KEEP_ALIVE]).then(apduResponse => {
      let status = Buffer.from(apduResponse.slice(apduResponse.length - 2)).readUInt16BE(0);
