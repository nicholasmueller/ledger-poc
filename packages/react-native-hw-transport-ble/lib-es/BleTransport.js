...
async function open(deviceOrId, needsReconnect) {
  ...
  if (typeof deviceOrId === "string") {
    ...
    if (!device) {
      const connectedDevices = await bleManager.connectedDevices(getBluetoothServiceUuids());
      const connectedDevicesFiltered = connectedDevices.filter(d => d.id === deviceOrId);
      log("ble-verbose", `found ${connectedDevicesFiltered.length} connected devices`);
...
export default class BluetoothTransport extends Transport {
  ...
  static listen(observer) {
    ...
    const stateSub = bleManager.onStateChange(async state => {
      if (state === "PoweredOn") {
        stateSub.remove();
        const devices = await bleManager.connectedDevices(getBluetoothServiceUuids());
        if (unsubscribed) return;
        await Promise.all(devices.map(d => BluetoothTransport.disconnect(d.id).catch(() => {})));
        if (unsubscribed) return;
        bleManager.startDeviceScan(getBluetoothServiceUuids(), null, (bleError, device) => {
          if (bleError) {
  ...
  constructor(device, writeCharacteristic, notifyObservable, deviceModel) {
    ...
    this.exchange = apdu => this.exchangeAtomicImpl(async () => {
      try {
        ...
        log("apdu", `=> ${msgIn}`);
        const data = await merge( // $FlowFixMe
  ...
  async requestConnectionPriority(connectionPriority) {
    await decoratePromiseErrors(this.device.requestConnectionPriority(ConnectionPriority[connectionPriority]));
  }
...
BluetoothTransport.isSupported = () => Promise.resolve(typeof BleManager === "function");
